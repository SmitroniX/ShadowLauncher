package com.shadowlauncher.modloaders;
...
public class FabriclikeDownloadTask implements Runnable, Tools.DownloaderFeedback{
    ...
    private boolean runCatching() throws IOException{
        ...
        if(mCreateProfile) {
            ...
            String cleanName = Tools.formatVersionInstanceName(versionId);
            fabricProfile.name = cleanName;
            fabricProfile.icon = mUtils.getIconName();
            fabricProfile.gameDir = Tools.generateInstancePath(cleanName, versionId);
            Tools.ensureInstanceDirectoryStructure(fabricProfile);
